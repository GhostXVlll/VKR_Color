...
# Рисователь меню
    def draw_menu(self):
        ...
        colorize_menu.add_command(label="Раскрасить", command=self.colorize)
        ...
        edit_menu.add_cascade(label="Автофильтр", menu=filter_menu)
        edit_menu.add_cascade(label="Отразить", menu=flip_menu)
        edit_menu.add_cascade(label="Изменить масштаб", menu=resize_menu)

        edit_menu.add_command(label="Заменить цвет", command=self.cr2)
...
# Открыть изображения (можно несколько враз)
    def open_new_images(self):
        image_paths = fd.askopenfilenames(filetypes=(("Images", "*.jpeg;*.jpg;*.img"), ))
...
# Добавление нового изображения
    def add_new_image(self, image_path):
        ...
        image_tab = Canvas(self.image_tabs)     # was "Frame"
...
# Замена цвета
    def change_color(self, event):
        ...
        current_pixel_color = image.getpixel(coordinate)
        percentage = 5
        #lower_border = tuple([x-5 for x in current_pixel_color])
        #higher_border = tuple([x+5 for x in current_pixel_color])
        lower_border = tuple([(x + x / 100 * percentage) for x in current_pixel_color])
        higher_border = tuple([(x - x / 100 * percentage) for x in current_pixel_color])
        ...
        for x in range(width):
            for y in range(height):
                ...
                if lower_border <= temp <= higher_border:
...
# вход в замену цвета
    def cr2(self):
        #cr_label = Label(text="Режим замены цвета: ")
        #cr_label.pack()
        #cr_cancel_button = Button(name="Выйти")
        #cr_cancel_button.pack()
